...
rest_days <- as.numeric((as.Date("2017-04-17") - min(workouts$Date)) - active_days)
runs <- workouts %>% filter(Activity == "run") 
mileage <- sum(runs$Distance_miles)
avg_run_pace <- round(mean(runs$Pace[!is.na(runs$Pace)]), 2)
avg_min_mile <- round(mean(runs$min_per_mile[!is.na(runs$min_per_mile)]), 0)
table(tolower(workouts$Stretch))
table(tolower(workouts$Indoor_outdoor))
table(tolower(workouts$Location))

print(paste0("Active: ", active_days))
print(paste0("Rest: ", rest_days))
print(paste0("Total Running Mileage: ", mileage))
print(paste0("Average run pace of ", avg_run_pace, " mph or roughly ", avg_min_mile, " minutes per mile."))
```
...
```{r}
plot1 <- ggplot(workouts, aes(Activity, fill = Activity)) +
  geom_bar(stat = "count") +
  theme_light() +
...
ggsave("~/Downloads/marathon.png", plot = plot1)
```
